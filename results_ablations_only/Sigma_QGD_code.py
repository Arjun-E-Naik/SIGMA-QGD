...
# ===================================================================
# >>> GPU: PennyLane device setup 
# ===================================================================

DISABLE_VSNG: bool = True
BENCHMARK_METHODS: List[str] = [
    "SIGMA-QGD v9.0", "Adam (best)", "COBYLA", "SPSA", "Diag-QNG", "QN-SPSA"
]
ABLATION_MAX_SEEDS: int = 10
...
def derive_config(n_qubits: int, reps: int, max_steps: int,
                  ...
                  lambda_reg: float = 0.01, tau: float = 1.0,
                  ema_window: Optional[int] = None) -> dict:
    p   = n_qubits * reps
    xxz = ham_type.lower() in ("xxz", "heisenberg", "xyz")
    # >>> Q3 sensitivity sweep: EMA window W normally auto-derives from
    # max_steps. `ema_window` lets the sweep override it directly.
    W      = int(ema_window) if ema_window is not None else min(30, max(15, max_steps // 10))
    W_med  = max(10, 3 * W)
...
class CurvatureProxy:
    ...
    def precondition(self, welford_raw: np.ndarray,
        ...
        F_diag = self.multiplier * welford_raw + self.lambda_reg
        return gradient / (F_diag + self.eps)

    def gamma(self, welford_raw: np.ndarray) -> float:
        F_mean      = float(self.multiplier * np.mean(welford_raw) + self.lambda_reg)
...
class SingleDimEscape:
    def __init__(self, cfg: dict, eta_0: float = 0.05,
                 ...
                 sigma_scale: float = 0.08, multiplier: float = 4.0):
...
class CostEvaluator:
    ...
    def _tape(self, theta: np.ndarray) -> qml.tape.QuantumScript:
        ...
        if self.shots is not None:
            ...
            tape = qml.tape.QuantumScript(
                tape.operations, tape.measurements, shots=self.shots)
        return tape

    def _batch_execute(self, tapes):

        try:
            return self.device.execute(tapes)
        except Exception:
            pass
        try:
    ...
    def statevector(self, theta: np.ndarray) -> np.ndarray:
        ...
        Always analytic, regardless of self.shots."""
        self.n_calls += 1
        with qml.tape.QuantumTape() as tape:
            ansatz_fn(theta, self.n_qubits, self.reps)
            qml.state()
...
def run_sigma_qgd_v9(cost_fn, theta_init: np.ndarray,
                     ...
                     precond_variance: str = "raw",
                     precond_multiplier: float = 4.0,
                     ema_window: Optional[int] = None,
                     logger: ParameterLogger = None,
                     seed: int = 0):
    """
    raw-vs-shrunk preconditioner ablation + sensitivity
    sweep. Three new knobs, all backward-compatible :
      precond_variance   : "raw"    -> preconditioner uses welford_raw
                                        (or welford_corrected under drift
                                        correction), i.e. sigma_hat_i^2
                                        (ORIGINAL behaviour, default).
                            "shrunk" -> preconditioner uses var_eng.js_shrunk,
                                        the James-Stein shrinkage estimate
                                        sigma_tilde_i^2, instead.
      precond_multiplier : the "4x" in F_diag = 4*sigma^2 + lambda_reg.
                            Sweepable for Q3 (default 4.0 = unchanged).
      ema_window         : override for the auto-derived EMA window W used
                            by VarianceEngine / NormalizedCUSUM. Sweepable
                            for Q3 (default None = unchanged auto-derivation).
    """
    if DISABLE_VSNG:
        use_vsng = False
    if precond_variance not in ("raw", "shrunk"):
        raise ValueError(f"precond_variance must be 'raw' or 'shrunk', got {precond_variance!r}")

    t0  = time.time()
    cfg = derive_config(n_qubits, reps, max_steps, ham_type,
                        eta_0, lambda_reg, tau, ema_window=ema_window)
    p   = cfg["p"]
    ...
    curvature = CurvatureProxy(eta_0, lambda_reg, multiplier=precond_multiplier)
    lscape    = LandscapeClassifier(cfg, eta_0)
    escape    = SingleDimEscape(cfg, eta_0, lambda_reg,
                                multiplier=precond_multiplier) if use_escape else None
    ...
    for t in range(1, max_steps + 1):
        ...
        precond_var_vec = (var_eng.js_shrunk if precond_variance == "shrunk"
                           else welford_for_precond)
        ...
        if use_precond and t >= cfg["t_qfim"]:
            d = curvature.precondition(precond_var_vec, mhat)
        else:
            d = mhat
        ...
        if use_escape and escape is not None:
            gamma_val = curvature.gamma(precond_var_vec)
            sigma_vec = curvature.per_dim_sigma(precond_var_vec, gamma_val)
            new_theta, new_cost, escaped = escape.escape(
                theta=theta, phase=phase, no_imp=lscape.no_imp,
                cusum_alarm=alarm, welford_raw=precond_var_vec,
...
def run_sigma_qgd_v8(cost_fn, theta_init, n_qubits, reps,
                     ...
                     precond_variance="raw", precond_multiplier=4.0,
                     ema_window=None,
                     logger=None, seed=0):
    tup = run_sigma_qgd_v9(cost_fn, theta_init, n_qubits, reps,
                           ...
                           precond_variance, precond_multiplier, ema_window,
                           logger, seed)
    return tup[0], tup[1]
...
def run_ablation(H: qml.Hamiltonian, ansatz_spec, ham_name: str, ham_type: str,
    ...
    configs = {
        ...
        "With annealing":      dict(escape=False, precond=True,  momentum=True,
                                    cusum=False,  vsng=False,     anneal=True,
                                    clip=True,    drift_correction=False),
        ...
        "With escape+CUSUM":   dict(escape=True,  precond=True,  momentum=True,
                                    cusum=True,   vsng=False,     anneal=False,
                                    clip=True,    drift_correction=False),
    }
...
def run_precond_variance_ablation(H: qml.Hamiltonian, ansatz_spec, ham_name: str,
                                  ham_type: str, n_qubits: int, reps: int,
                                  max_steps: int, n_seeds_abl: int = 5,
                                  logger: ParameterLogger = None) -> Dict:
    n_seeds_abl = min(n_seeds_abl, ABLATION_MAX_SEEDS)
    print(f"\n{'='*78}\n  PRECONDITIONER ABLATION: raw sigma_hat^2 vs shrunk sigma_tilde^2"
          f"\n  {ham_name}  |  {n_qubits}q  |  {n_seeds_abl} seeds  |  [Backend]={_GPU_STATUS}"
          f"\n{'='*78}")

    exact_E  = exact_gs(H)
    n_params = n_qubits * reps

    # Identical to the "Full v9.0" row in run_ablation(); only precond_variance varies.
    base_flags = dict(use_escape=False, use_precond=True, use_momentum=True,
                      use_cusum=False, use_vsng=False, use_anneal=False,
                      use_clip=True, use_drift_correction=False)
    configs = {
        "Precond: raw (sigma_hat^2)":    dict(precond_variance="raw"),
        "Precond: shrunk (sigma_tilde^2)": dict(precond_variance="shrunk"),
    }
    ...
    for seed in range(n_seeds_abl):
        ...
        for cname, extra in configs.items():
            ...
            tup = run_sigma_qgd_v9(
                cf, theta0.copy(), n_qubits=n_qubits, reps=reps,
                max_steps=max_steps, ham_type=ham_type,
                precond_variance=extra["precond_variance"],
                logger=logger, seed=seed, **base_flags)
            results[cname].append(tup[0])

    base_key  = "Precond: raw (sigma_hat^2)"
    base_gaps = [abs(r.energy_opt - exact_E) for r in results[base_key]]
    base_gap  = float(np.mean(base_gaps))

    print(f"\n  {'Config':<34} {'Gap mean':>10} {'Gap std':>10} "
          f"{'Circuits':>10} {'Ratio':>8} {'p-val':>8}")
    print(f"  {'─'*34} {'─'*10} {'─'*10} {'─'*10} {'─'*8} {'─'*8}")

    sig_rows, csv_rows = [], []
    for cname, rs in results.items():
        gaps  = [abs(r.energy_opt - exact_E) for r in rs]
        circs = [r.n_circuits for r in rs]
        ratio = float(np.mean(gaps)) / max(base_gap, 1e-12)
        tag   = " (base)" if cname == base_key else f" {ratio:.2f}x"
        pval_str = "  (base)"
        if cname != base_key and len(base_gaps) >= 3 and len(gaps) >= 3:
            n_m = min(len(base_gaps), len(gaps))
            diffs = np.array(gaps[:n_m]) - np.array(base_gaps[:n_m])
            try:
                stat, pval = scipy_stats.wilcoxon(diffs, alternative="two-sided")
                pval_str = f"  {pval:.4f}"
                sig_rows.append(dict(ham=ham_name, config=cname,
                ...
                pval_str = "  N/A"
        print(f"  {cname:<34} {np.mean(gaps):>10.5f} {np.std(gaps):>10.5f} "
              f"{np.mean(circs):>10.0f}{tag}{pval_str}")
        csv_rows.append(dict(config=cname, gap_mean=round(float(np.mean(gaps)), 6),
                             gap_std=round(float(np.std(gaps)), 6),
                             circuits_mean=round(float(np.mean(circs)), 1),
                             ratio=round(ratio, 4), n_seeds=len(rs)))

    os.makedirs("sigma_data", exist_ok=True)
    path = "sigma_data/precond_variance_ablation.csv"
    with open(path, "w", newline="") as f:
        w = csv.DictWriter(f, fieldnames=["config", "gap_mean", "gap_std",
                                          "circuits_mean", "ratio", "n_seeds"])
        w.writeheader(); w.writerows(csv_rows)
    print(f"  [Precond Ablation] Saved -> {path}")

    if sig_rows:
        sig_path = "sigma_data/precond_variance_significance.csv"
        with open(sig_path, "w", newline="") as f:
            w = csv.DictWriter(f, fieldnames=["ham", "config", "statistic",
                                              "p_value", "ratio", "n"])
            w.writeheader(); w.writerows(sig_rows)
        print(f"  [Precond Ablation] Significance test -> {sig_path}")
...
def run_sensitivity_sweep(H: qml.Hamiltonian, ansatz_spec, ham_name: str,
                          ham_type: str, n_qubits: int, reps: int,
                          max_steps: int, n_seeds: int = 5,
                          multiplier_values: Tuple[float, ...] = (2.0, 4.0, 6.0, 8.0),
                          lambda_reg_values: Tuple[float, ...] = (0.001, 0.01, 0.05, 0.1),
                          ema_window_values: Tuple[int, ...] = (10, 20, 30, 45),
                          base_eta: float = 0.05,
                          base_lambda: float = 0.01,
                          base_multiplier: float = 4.0,
                          logger: ParameterLogger = None) -> List[dict]:
    n_seeds  = min(n_seeds, ABLATION_MAX_SEEDS)
    exact_E  = exact_gs(H)
    n_params = n_qubits * reps
    rows: List[dict] = []

    print(f"\n{'='*78}\n  SENSITIVITY SWEEP -- {ham_name}  |  {n_qubits}q  |  {n_seeds} seeds"
          f"\n  base: multiplier={base_multiplier}  lambda_reg={base_lambda}  "
          f"eta_0={base_eta}  ema_window=auto"
          f"\n{'='*78}")

    def run_point(sweep: str, value, multiplier: float, lambda_reg: float,
        ...
        for seed in range(n_seeds):
            ...
            theta0 = np.random.uniform(-np.pi/8, np.pi/8, n_params)
            cf = CostEvaluator(n_qubits, reps, H)
            res, _rec, _ls, _cu, _es = run_sigma_qgd_v9(
                cf, theta0.copy(), n_qubits=n_qubits, reps=reps,
                max_steps=max_steps, ham_type=ham_type,
                eta_0=base_eta, lambda_reg=lambda_reg, tau=1.0,
                use_escape=False, use_precond=True, use_momentum=True,
                use_cusum=False, use_vsng=False, use_anneal=False,
                use_clip=True, use_drift_correction=False,
                precond_variance="raw", precond_multiplier=multiplier,
                ema_window=ema_window, logger=logger, seed=seed)
            gaps.append(abs(res.energy_opt - exact_E))
            circs.append(res.n_circuits)
        row = dict(sweep=sweep, value=value,
                  gap_mean=float(np.mean(gaps)), gap_std=float(np.std(gaps)),
                  circuits_mean=float(np.mean(circs)), n_seeds=n_seeds)
        rows.append(row)
        print(f"    {sweep:<12} = {str(value):<6}  gap={row['gap_mean']:.5f} "
              f"+/- {row['gap_std']:.5f}   circuits={row['circuits_mean']:.0f}")
        return row

    print("\n  -- multiplier sweep (F_diag = mult * sigma_hat^2 + lambda_reg) --")
    for mv in multiplier_values:
        run_point("multiplier", mv, mv, base_lambda, None)

    print("\n  -- lambda_reg sweep --")
    for lv in lambda_reg_values:
        run_point("lambda_reg", lv, base_multiplier, lv, None)

    print("\n  -- EMA window sweep --")
    for wv in ema_window_values:
        run_point("ema_window", wv, base_multiplier, base_lambda, wv)

    os.makedirs("sigma_data", exist_ok=True)
    path = "sigma_data/sensitivity_sweep.csv"
    with open(path, "w", newline="") as f:
        w = csv.DictWriter(f, fieldnames=["sweep", "value", "gap_mean",
                                          "gap_std", "circuits_mean", "n_seeds"])
        w.writeheader(); w.writerows(rows)
    print(f"\n  [Sensitivity] Saved -> {path}")

    # small compact chart, one panel per swept parameter
    try:
        fig, axes = plt.subplots(1, 3, figsize=(15, 4))
        for ax, sweep_name, xlabel in zip(
                axes, ["multiplier", "lambda_reg", "ema_window"],
                ["multiplier (x in F=x*sigma^2+lambda_reg)", "lambda_reg", "EMA window W"]):
            sub = [r for r in rows if r["sweep"] == sweep_name]
            xs  = [r["value"] for r in sub]
            ys  = [r["gap_mean"] for r in sub]
            es  = [r["gap_std"] for r in sub]
            ax.errorbar(xs, ys, yerr=es, marker="o", color="#00C9A7", capsize=4)
            ax.set_xlabel(xlabel)
            ax.set_ylabel("Energy gap (mean +/- std)")
            ax.set_title(f"Sensitivity: {sweep_name}")
            ax.grid(alpha=0.3)
        fig.suptitle(f"Q3 sensitivity sweep -- {ham_name}, {n_seeds} seeds")
        fig.tight_layout()
        fig_path = "sigma_data/fig_sensitivity_sweep.png"
        fig.savefig(fig_path, dpi=150)
        plt.close(fig)
        print(f"  [Sensitivity] Figure -> {fig_path}")
    except Exception as e:
        print(f"  [Sensitivity] Figure generation skipped ({e})")
...
def test_shot_noise_actually_varies():
    ...
    H = build_tfim(n)
    theta = np.random.uniform(-0.5, 0.5, n * r)
    cf_100  = ShotNoiseCostEvaluator(n, r, H, n_shots=100)
    cf_5000 = ShotNoiseCostEvaluator(n, r, H, n_shots=5000)
    vals_100  = [cf_100(theta) for _ in range(5)]
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--precond-ablation", action="store_true",
                    help="Q2: raw sigma_hat^2 vs shrunk sigma_tilde^2 preconditioner ablation")
    ap.add_argument("--sensitivity-sweep", action="store_true",
                    help="Q3: OFAT sweep over precond multiplier (4x), lambda_reg, EMA window")
    ap.add_argument("--shot-noise",      action="store_true")
    ap.add_argument("--scaling",         action="store_true")
    ap.add_argument("--scaling-qubits",  type=str, default="4,8,10,12")
...
if __name__ == "__main__":
    ...
    if args.run_tests:
        ...
        t3 = test_shot_noise_actually_varies()
        t4 = test_no_xxz_fallback()
    ...
    if args.precond_ablation:
        H_pa = build_tfim(args.qubits)
        precond_ablation_results = run_precond_variance_ablation(
            H_pa, ansatz_spec, "TFIM", "tfim",
            args.qubits, args.reps, args.steps,
            n_seeds_abl=args.seeds, logger=logger)

    if args.sensitivity_sweep:
        H_ss = build_tfim(args.qubits)
        sensitivity_rows = run_sensitivity_sweep(
            H_ss, ansatz_spec, "TFIM", "tfim",
            args.qubits, args.reps, args.steps,
            n_seeds=args.seeds, base_lambda=args.lam, base_eta=args.eta,
            logger=logger)
    ...
    if precond_ablation_results:
        print("  sigma_data/precond_variance_ablation.csv     -- Q2: raw vs shrunk preconditioner")
        print("  sigma_data/precond_variance_significance.csv -- Q2: Wilcoxon test")
    if sensitivity_rows:
        print("  sigma_data/sensitivity_sweep.csv       -- Q3: multiplier/lambda_reg/EMA sweep")
        print("  sigma_data/fig_sensitivity_sweep.png   -- Q3: compact sweep chart")
    if shot_results:
    ...
    print("  Q2 (raw vs shrunk precond, 10q/5-seed):")
    print("                python Sigma_QGD_pennylane.py --qubits 10 --seeds 5 --precond-ablation")
    print("  Q3 (sensitivity sweep, 10q/5-seed, TFIM only):")
    print("                python Sigma_QGD_pennylane.py --qubits 10 --seeds 5 --sensitivity-sweep")
